...
def rescale_arguments(args, benchmark):
    ...
    elif benchmark == 'imagenet_resnet_512':
        lr_decay_factor_range =  0.010294, 0.989753
        lr_initial_range =  0.000010, 9.774312
        lr_power_range =  0.100225, 1.999326
...
class PD1Function(BaseTestProblem):
    ...
    def __init__(self, task_name: str, noise_std: float = None, negate: bool = False, seed: int = 42):
        self.seed = seed
        self.dim = 4
        self._bounds = [(0.0, 1.0) for _ in range(self.dim)]
        self.bench = mfpbench.get(task_name, seed=seed, datadir=DATADIR)
        self.task_name = task_name
        super().__init__(noise_std=noise_std, negate=negate)

    def evaluate_true(self, X: Tensor, seed=None) -> Tensor:
        args_dict = rescale_arguments(tuple(*X.detach().numpy()), self.task_name)
