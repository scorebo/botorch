...
class GeneralizedLinearPath(SamplePath):
    ...
    def __init__(
        ...
        input_transform: Optional[TInputTransform] = None,
        output_transform: Optional[TOutputTransform] = None,
        additive_components: Optional[Tensor] = None,
    ):
    ...
    def forward(self, x: Tensor, **kwargs) -> Tensor:
        ...
        if isinstance(self.feature_map, AdditiveKernelFeatureMap):
            dim_diff = 1 + feat.ndim - self.weight.unsqueeze(-1).ndim
            weight_shape = Size([1] * dim_diff) + self.weight.shape + Size([1])
        
            feat = feat.unsqueeze(1)
            dim_diff = feat.ndim - self.weight.unsqueeze(-1).ndim
            weight_shape = Size([1] * dim_diff) + self.weight.shape + Size([1])
            out = (feat @ self.weight.view(weight_shape)).squeeze(-1)
            
            out = out.sum(dim=0).squeeze(0)
            
        else:
            out = (feat @ self.weight.unsqueeze(-1)).squeeze(-1)
        #print(type(self.feature_map), out.shape)
