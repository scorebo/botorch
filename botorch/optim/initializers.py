...
def gen_batch_initial_conditions(
    ...
    suggestions = get_suggestions(acq_function)
    ...
    while factor < max_factor:
        with warnings.catch_warnings(record=True) as ws:
            ...
            if suggestions is not None:
                X_suggest = sample_truncated_normal_perturbations(
                    X=suggestions,
                    n_discrete_points=n * q,
                    sigma=options.get("sample_around_suggestion_simga", 1e-2),
                    bounds=bounds,
                )
                if X_suggest is not None:
                    X_rnd = torch.cat(
                        [
                            X_rnd,
                            X_suggest.view(n, q, bounds.shape[-1]).cpu(),
                        ],
                        dim=0,
                    )
...
def gen_jes_initial_conditions(
    acq_function: qJointEntropySearch,
    bounds: Tensor,
    q: int,
    num_restarts: int,
    raw_samples: int,
    ...
    This function generates initial conditions for optimizing JES. Intutively, 
    the maximizer of JES will often be close to one or more of the sampled optima.
    This function uses that fact to generate the initital conditions for the 
    acquisition function. Specifically, a fraction of `1 - frac_random` (see
    options) is generated by sampling from the set of sampled optima. The remaining 
    `frac_random` are chosen according to the standard initialization strategy in
    `gen_batch_initial_conditions`.

    Args:
        acq_function: The qJointEntropySearch instance to be optimized.
        bounds: A `2 x d` tensor of lower and upper bounds for each column of
            task features.
        q: The number of candidates to consider.
        num_restarts: The number of starting points for multistart acquisition
    ...
    """
    options = options or {}
    frac_random: float = options.get("frac_random", 0.5)
    sigma: float = options.get("sigma", 0.01)
    if not 0 < frac_random < 1:
        raise ValueError(
            f"frac_random must take on values in (0,1). Value: {frac_random}"
        )
    num_optima = acq_function.optimal_outputs.numel()
    extra_per_optimum = round((1 - frac_random) * raw_samples / num_optima)
    perturbations = sigma * \
        draw_sobol_samples(bounds, num_optima * extra_per_optimum,
                           q).reshape(extra_per_optimum, num_optima, -1)
    # stack all the optimal inputs to the first dimension if there are multiple,
    # squeeze the second-last one to enable usage of the sampling method
    inputs = acq_function.optimal_inputs.reshape(num_optima, -1)
    perturbed_inputs = torch.clamp(inputs + perturbations, bounds[0], bounds[1]
                                   ).reshape(extra_per_optimum * num_optima, -1)
    ics = gen_batch_initial_conditions(
        acq_function=acq_function,
        bounds=bounds,
        q=q,
        num_restarts=num_restarts,
        raw_samples=raw_samples - len(perturbed_inputs),
        fixed_features=fixed_features,
        options=options,
        inequality_constraints=inequality_constraints,
        equality_constraints=equality_constraints,
...
def sample_truncated_normal_perturbations(
    ...
    """
    n_points_per_X = ceil(n_discrete_points / len(X))
    d = X.shape[1]
    X = normalize(X, bounds=bounds).unsqueeze(1)
    # sample points from N(X_center, sigma^2 I), truncated to be within
    # [0, 1]^d.
    if qmc:
        std_bounds = torch.zeros(2, d, dtype=X.dtype, device=X.device)
        std_bounds[1] = 1
        u = draw_sobol_samples(bounds=std_bounds, n=n_points_per_X, q=1).squeeze(1)
    else:
        u = torch.rand((n_points_per_X, d), dtype=X.dtype, device=X.device)
    ...
    return unnormalize(perturbed_X.reshape(n_points_per_X * len(X), d)[:n_discrete_points], bounds=bounds)
